...
Drag, swipe, or tap a tile then tap an adjacent one to create matches:
...
- Mouse, touch/swipe, and tap-to-select controls
- Responsive home screen and game HUD
- Vertically centered home page content on mobile
- Stylish, prominent game title
- Accessible UI: visible focus styles, ARIA roles, keyboard navigation, tap-to-select board interaction for screen readers (e.g. Android TalkBack) and keyboard users
...
├── styles/
│   ├── base.css
│   ├── fonts/          # self-hosted webfonts (no runtime network dependency)
│   ├── game.css
│   ├── home.css
│   └── modals.css
│
...
│
├── coverage/             # generated by `npx jest --coverage`, gitignored
├── www/                  # generated by `npm run build`, gitignored — Capacitor's webDir
├── index.html
...
- Generate a release keystore and produce a signed Android App Bundle.
- Prepare Play Store assets such as feature graphic, screenshots, and store copy.
